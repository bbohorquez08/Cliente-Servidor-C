...
int Buscar_Indice_Cliente(int prmSocketCliente);
...
int Buscar_Indice_Cliente(int prmSocketCliente){
     for (int i = 0; i < MAX_CLIENTES; i++){
        if (array_cliente[i] == prmSocketCliente){
            return i;
        }
    }
    return -1;
...
void * controlador_conexion(void * c){
    ...
    while(1){
        ...
        printf("Leido del proceso cliente %d (%d bytes): %s %s\n",(Buscar_Indice_Cliente(client_s)+1), nread, header->operation, header->filename);
        fd=0;
...
void get(mensaje *prmHeader,int prmClient_Socket,int fd,char *direccion,struct stat stats){
       //Se verifica la existencia del archivo
            ...
            if (stats.st_size <= 0){
                printf("No se puede enviar el archivo [%s] al cliente [%d] ya que este archivo se encuentra vacio\n",prmHeader->filename,(Buscar_Indice_Cliente(prmClient_Socket))+1);
                return;
            }
            ...
            close(fd);
            printf("Archivo enviado al cliente: %d\n",(Buscar_Indice_Cliente(prmClient_Socket))+1);
}
void controlador_siguiente(int argc){
    if (argc == SIGINT){
